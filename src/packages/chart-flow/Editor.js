...
class Editor extends Events {
  ...
  registerCatagory(cata) {
    ...
    catagory.set(cata.id, cata.label)
  }
  ...
  async registerNodeType(func) {
    // 注册节点类型模板
    let RealNodeType = await func.call(null)
  ...
  getNodeCatagory() {
    // 返回注册的节点类别
    let map = this.___def.NodeCatagory
    // if (!set.size) {
  ...
  updateNodeDescription(nodeId, desc) {
    util.updateNodeDescription(this, nodeId, desc)
  }

  updateNodeIsChanged(nodeId, show) {
    util.updateNodeIsChanged(this, nodeId, show)
  }

  updateNodeIsErrored(nodeId, show) {
